...
class OrderRepository:
    ...
    async def find_all(self) -> List[Order]:
        res = await self.session.execute(
            text("""
                SELECT 
                    o.id as order_id, o.user_id, o.status, o.total_amount, o.created_at
                    oi.id as item_id, oi.product_name, oi.price, oi.quantity,
                    h.id as history_id, h.status as h_status, h.changed_at
                FROM orders AS o
        ...
        for row in rows:
            ...
            order_obj.total_amount = Decimal(str(row["total_amount"]))
            ...
            history = OrderStatusChange(
                id=row["history_id"],
                order_id=order_id,
                status=OrderStatus(row["h_status"]),
                changed_at=row["changed_at"]
            )
